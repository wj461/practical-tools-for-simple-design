...
void Map::Update() {
    ...
    glm::vec2 index_pos{floor(mouse_position.x / BLOCK_SIZE),
...
void Map::LoadEmptyMap() {
    for (glm::int64 y = 0; y < MAP_SIZE.y; y++) {
        ...
        for (glm::int64 x = 0; x < MAP_SIZE.x; x++) {
            glm::vec2 index_pos = {MAP_START_INDEX.x + x,
                                   MAP_START_INDEX.y - y};
    ...
    for (unsigned long i = 0; i < material_path.size(); ++i) {
        ...
        for (unsigned long j = 0; j < material_path[i].size(); ++j) {
            glm::vec2 material_position = {
                ...
                LEFT_TOP_POS.y -
                    (glm::int64(j / MATERIAL_COL_NUM) * BLOCK_SIZE),
            };

            glm::vec2 index_pos{floor(material_position.x / BLOCK_SIZE) + 1,
                                floor(material_position.y / BLOCK_SIZE) - 1};
...
void Map::LoadChooseMaterialFocus() {
    ...
    auto img = std::make_shared<Util::Image>(path);
    material_focus =
        NewBlock(MATERIAL_START_INDEX, {0, 0}, BlockType::Focus, FOCUS_Z, img);
}
...
void Map::LoadChooseToolFocus() {
    ...
    auto img = std::make_shared<Util::Image>(path);
    tool_focus =
        NewBlock(TOOL_START_INDEX, {0, 0}, BlockType::Focus, FOCUS_Z, img);
}

void Map::LoadPageFocus() {
    auto path = SPRITES_PATH + "focus24.png";
    auto img = std::make_shared<Util::Image>(path);
    page_focus = NewBlock(MATERIAL_PAGE_START_INDEX, {0, 0}, BlockType::Focus,
                          FOCUS_Z, img);
}
...
void Map::LoadToolImage() {
    std::vector<std::string> tool_img_names = {"pan.png", "mouse.png"};
    ...
    for (unsigned long i = 0; i < tool_img_names.size(); ++i) {
        ...
        auto tool_block =
            NewBlock({TOOL_START_INDEX.x + i, TOOL_START_INDEX.y}, {0, 0},
                     BlockType::ToolIcon, MAP_Z, tool_icon);
        tools.push_back(tool_block);
    }
}

std::shared_ptr<Block> Map::NewBlock(glm::vec2 indexPos, glm::vec2 indexMap,
                                     BlockType type, glm::int64 indexZ,
...
std::shared_ptr<WithTextButton> Map::NewTextButton(glm::vec2 indexPos,
                                                   std::string text) {
...
bool Map::IsEditRange(glm::vec2 indexPos) {
    if (indexPos.x >= MAP_START_INDEX.x && indexPos.x < MAP_END_INDEX.x &&
        indexPos.y <= MAP_START_INDEX.y && indexPos.y > MAP_END_INDEX.y) {
        return true;
    }
...
glm::vec2 Map::ChooseMaterial(glm::vec2 indexPos, glm::int64 currentPage) {
    ...
    glm::int64 material_index =
        ...
        (MATERIAL_COL_NUM *
         (MATERIAL_START_INDEX.y - (glm::int64(indexPos.y))));
...
void Map::ChangeBlockMaterial(glm::vec2 indexPos, int indexMap) {
    ...
    block->SetIndexMaterial(
        {current_page_index, indexMap},
...
glm::int64 Map::ChoosePage(glm::vec2 indexPos) {
    ...
    if (index < glm::int64(material_path.size()) &&
        indexPos.y == MATERIAL_PAGE_START_INDEX.y) {
