...
void WithTextButton::SetTextPostionToCenter(){
    ...
    position.x + this->GetSize().x/2 + 6,
    position.y + this->GetSize().y/2 + 12
    });
    LOG_DEBUG("T P `{} {}", m_TextObject->GetTransform().translation.x, m_TextObject->GetTransform().translation.y);
}