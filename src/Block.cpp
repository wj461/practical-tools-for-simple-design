...
void Block::SetIndexMaterial(glm::vec2 index,
                             std::shared_ptr<Util::Image> material,
                             bool canStand) {
    index_material[index.x] = index.y;
    stand = canStand;

    zBlock[index.x]->SetDrawable(material);
    zBlock[index.x]->SetPivotToLeftButton();
}

void Block::SetIndexPostion(glm::vec2 indexPos) {
    glm::vec2 translation{(indexPos.x * BLOCK_SIZE), (indexPos.y * BLOCK_SIZE)};
...
void Block::CreatNewZBlock() {
    glm::vec2 translation{(index_pos.x * BLOCK_SIZE),
